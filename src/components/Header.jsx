    ...
    <header className="w-full px-4 py-4 transition-colors bg-white border-b border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700 md:px-6">
      <div className="flex flex-col justify-between w-full max-w-full lg:flex-row lg:items-center">
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold text-gray-800 truncate md:text-2xl dark:text-white">{title}</h1>
          {subtitle && <p className="mt-1 text-sm text-gray-600 truncate md:text-base dark:text-gray-300">{subtitle}</p>}
        </div>
        ...
        <div className="flex items-center flex-shrink-0 mt-4 space-x-2 md:space-x-4 lg:mt-0">
          ...
          <button
            onClick={toggleTheme}
            className="relative flex-shrink-0 p-2 text-gray-600 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
            title={isDark ? "Switch to light mode" : "Switch to dark mode"}
            aria-label="Toggle dark mode"
          >
            <div className="relative w-5 h-5">
              {isDark ? (
                <AiOutlineSun size={20} className="absolute inset-0 text-yellow-400" />
              ) : (
                <AiOutlineMoon size={20} className="absolute inset-0 text-gray-600" />
              )}
            </div>
          </button>

          {/* Notification */}
          <button className="relative flex-shrink-0 p-2 text-gray-600 transition-colors rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
